...
static int build_vf(VPEngine* vp) {
    ...
    snprintf(args, sizeof(args),
        "video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:frame_rate=%d/%d:pixel_aspect=%d/%d:colorspace=%d:range=%d",
        vp->vctx->width, vp->vctx->height, (int)vp->vctx->pix_fmt,
        tb.num, tb.den, fr.num ? fr.num : 25, fr.den ? fr.den : 1,
        sar.num, sar.den, (int)vp->vctx->colorspace, (int)vp->vctx->color_range);
    ...
    /* NOTE: FFmpeg 8.x made buffersink's pix_fmts an init-only option, so it
       can't be set after creation. We pin RGB24 by ending the chain with
       format=rgb24 instead (the auto-inserted scaler converts to it). */
    ...
    if (need_scale && user_vf[0])
        snprintf(fs_buf, sizeof(fs_buf), "scale=%d:%d,%s,format=rgb24", vp->out_w, vp->out_h, user_vf);
    else if (need_scale)
        snprintf(fs_buf, sizeof(fs_buf), "scale=%d:%d,format=rgb24", vp->out_w, vp->out_h);
    else if (user_vf[0])
        snprintf(fs_buf, sizeof(fs_buf), "%s,format=rgb24", user_vf);
    else
        snprintf(fs_buf, sizeof(fs_buf), "format=rgb24");
...
static int build_af(VPEngine* vp) {
    ...
    /* Pin the DEVICE format/rate/layout by ENDING the chain with aformat.
       (FFmpeg 8.x: abuffersink's sample_fmts/sample_rates/ch_layouts are
       init-only and can't be set after the sink is created.) This makes the
       graph resample to whatever the device opened with, so audio-filter swaps
       and track switches always feed miniaudio the right shape. */
    char aforce[160];
    {
        ...
        snprintf(aforce, sizeof(aforce),
                 "aformat=sample_fmts=flt:sample_rates=%d:channel_layouts=%s",
                 vp->pcm_sr, wbuf);
    ...
    char pre[1200];
    if (tempo[0] && user_af[0]) snprintf(pre, sizeof(pre), "%s,%s", tempo, user_af);
    else if (tempo[0])          snprintf(pre, sizeof(pre), "%s", tempo);
    else if (user_af[0])        snprintf(pre, sizeof(pre), "%s", user_af);
    else                        pre[0] = 0;

    char fsbuf[1400];
    if (pre[0]) snprintf(fsbuf, sizeof(fsbuf), "%s,%s", pre, aforce);
    else        snprintf(fsbuf, sizeof(fsbuf), "%s", aforce);
    const char* fs = fsbuf;
